...
while running:
    ...
    # Draw the highway
    pygame.draw.rect(screen, gray, (lane_x, lane_y, lane_width, lane_height))

    # Draw the road lines if there is more than one lane
    for lane in range(num_lanes - 1):
        line_x = lane_x + (lane_width - line_width) // num_lanes * (lane + 1)
...
# class Car:
#     def __init__(self, y):
#         self.x = random.randint(0, width - car_radius * 2)
#         self.y = y
#         self.color = red
#
#     def move(self):
